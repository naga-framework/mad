...
app(["create" , Name]) -> 
    ...
    %io:format("TplName = ~p~n",[TplName]),
    Binary = proplists:get_value("priv/." ++ TplName, Apps),
    TemplateTerms = consult(Binary),
    create(Apps, TemplateTerms, Name).

    %% [ case string:str(File,"priv/naga") of
    %%    1 -> Relative = Name ++ string:substr(File, 10),
    %%         io:format("Create File: ~p~n",[Relative]),
    %%         filelib:ensure_dir(Relative),
    %%         file:write_file(Relative,Bin);
    %%    _ -> skip
...
create(Files, FinalTemplate, Name) ->

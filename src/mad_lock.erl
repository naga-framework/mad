...
read_all_deps(Config, Dir) ->
    ...
    DepsDir = filename:join([mad_utils:get_value(deps_dir, Conf1, ["deps"])]), %%?? FIXME only for top app
    DepPath = filename:join([Dir, DepsDir]),
    Acc = [{top, dep_names(TopDeps)}],
    DepDirs = filelib:wildcard(filename:join(DepPath, "*")),
    Acc ++ [
...
extract_deps(Cwd) ->
    DepConfigFile = filename:join(Cwd, "rebar.config"),
    Config = mad_utils:consult(DepConfigFile),
    %%Fix exec script on conf
    Conf1 = mad_script:script(DepConfigFile, Config, none),    
    case lists:keyfind(deps, 1, Conf1) of
        {deps, Deps} -> Deps;
        false -> []
    end.
